...
def extend_shift_schedule(dates: List[str], shifts: List[str],
                         extension_days: int = get_default_continuation_days(),
                         pdf_title: str = "") -> Tuple[List[str], List[str], Dict[str, int]]:
    """
    ...
    year_tracker.extend_years_for_future_dates(extended_dates, extension_days)
    ...
    if has_errors:
        ...
        for shift_type, counts in shift_validation.items():
            if counts['expected'] > 0 and counts['actual'] != counts['expected']:
                missing = counts['expected'] - counts['actual']
                print(f"    ✗ {shift_type}: {counts['actual']}/{counts['expected']} "
                      f"(MISSING {missing})")
